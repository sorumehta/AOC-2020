...
def getNextPosition(pos: Tuple[int, int], col_delta, row_delta,  n_rows, n_cols) -> Tuple[int, int]:
    ...
    for slope in slopes:
        ...
        while position[0] < len(areaMap):
            if areaMap[position[0]][position[1]] == 1:
                n_trees += 1
            position = getNextPosition(position, slope[0], slope[1], len(areaMap), len(areaMap[0]))
        answer *= n_trees
    return answer
